...
const TaskListTable = ({tableData}) => {
    ...
    const getStatusBadgeColor = (status) => {
        switch (status) {
            ...
            case 'Pending': return 'bg-pruple-100 text-pruple-500 border border-pruple-200';
            ...
            <tbody>
                {tableData.map((task) => (
                    <tr key={task._id} className='border-t border-gray-200'>
                        ...
                        <td className="py-4 px-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell">{task.createdAt ? momemt(task.createdAt).format('Do MMM YYYY') : 'N/A'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
